...
public class EscalatorRamp extends AEscalatorRamp {
    ...
    public Map<Location, ItemStack> getOldBlock() {
        oldBlock.keySet().forEach(key -> AsyncBlocksSet.setBlockSuperFast(key.getBlock(), 0, (byte) 0, false));
    ...
    public void south() {
        ...
        while (i <= numberOfBlock) {
            getInstance().getServer().getScheduler().runTaskLater(getInstance(), () -> oldBlock.forEach((k, v) -> {
                AsyncBlocksSet.setBlockSuperFast(k.getBlock(), 0, (byte) 0, false);
                k.add(0, 0, 1);
                AsyncBlocksSet.setBlockSuperFast(k.getBlock(), v.getTypeId(), v.getData().getData(), false);
            ...
            getInstance().getServer().getScheduler().runTaskLater(getInstance(), new Runnable() {
                ...
                public void run() {
                    oldBlock.forEach((k, v) -> {
                        ...
                        if (k.getBlockZ() == minValue) AsyncBlocksSet.setBlockSuperFast(k.getBlock(), 0, (byte) 0, false);
                        k.add(0, 0, -1);
                        AsyncBlocksSet.setBlockSuperFast(k.getBlock(), v.getTypeId(), v.getData().getData(), false);
                    });
                }
            ...
            getInstance().getServer().getScheduler().runTaskLater(getInstance(), new Runnable() {
                ...
                public void run() {
                    oldBlock.forEach((k, v) -> {
                        ...
                        if (k.getBlockX() == minValue) AsyncBlocksSet.setBlockSuperFast(k.getBlock(), 0, (byte) 0, false);
                        k.add(-1, 0, 0);
                        AsyncBlocksSet.setBlockSuperFast(k.getBlock(), v.getTypeId(), v.getData().getData(), false);
                    });
                }
